...
"use strict";var t=require("lit-element"),e=a(require("../State.js")),i=require("../utils.js"),s=a(require("./BaseElement.js"));function a(t){return t&&t.__esModule?t:{default:t}}class r extends s.default{static get styles(){return t.css`
      ${super.styles}
      input {
        border: none;
        height: auto;
    ...
    `}constructor(){super(),this.timer=null,this.editing=!1,window.addEventListener("statechange",async t=>{const i=await e.default.getState();i.currentTimer&&(this.timer=await e.default.getTimerById(i.currentTimer),this.update())}),window.addEventListener("tick",async t=>{if(!this.editing){const t=await e.default.getState();this.timer=await e.default.getTimerById(t.currentTimer),this.update()}}),this.addEventListener("blur",t=>{this.saveTimer()})}deleteTimer(){e.default.deleteTimer(this.timer.id)}editTimer(){this.editing=!0,this.update(),this.shadowRoot.querySelectorAll(".edit-field")[0].focus()}saveTimer(){this.editing=!1,this.update()}playPauseTimer(t){0==t.state?e.default.startTimer(t.id):1==t.state?e.default.stopTimer(t.id):2==t.state&&(e.default.resetTimer(t.id),e.default.startTimer(t.id))}resetTimer(t){e.default.resetTimer(t.id)}updateTimer(t,i){const s=null==i.title?t.title:i.title,a=null==i.seconds?Math.floor(t.length/1e3)%60:i.seconds,r=null==i.minutes?Math.floor(t.length/60/1e3)%60:i.minutes,n=null==i.hours?Math.floor(t.length/60/60/1e3):i.hours;e.default.updateTimer(t.id,{title:s,length:1e3*a+60*r*1e3+60*n*60*1e3})}render(){const e=this.timer;if(!e)return t.html`
        <div class="header">
      ...
      `;let s=e.length;this.editing||(s=(0,i.getTimerTime)(e));const a=Math.floor(s/1e3)%60,r=Math.floor(s/60/1e3)%60,n=Math.floor(s/60/60/1e3);return t.html`
      <div class="header">
        <div class="header-left">
      ...
      <div class="controls">
        ...
        <button class="big" @click="${t=>this.playPauseTimer(e)}">
          ${1===e.state?t.html`<svg xmlns="http://www.w3.org/2000/svg" style="fill:#fff" width="55" height="66" viewBox="0 0 55 66"><g transform="translate(-0.056 -0.219)"><rect width="20" height="66" transform="translate(0.056 0.219)"/><rect width="20" height="66" transform="translate(35.056 0.219)"/></g></svg>`:t.html`<svg xmlns="http://www.w3.org/2000/svg" style="fill:#fff" width="50" height="60" viewBox="0 0 50 60"><g transform="translate(-692.714 -1280.057)"><path d="M0,0,50,30h0L0,60Z" transform="translate(692.714 1280.057)"/></g></svg>`}          
        </button>
      </div>
    `}}customElements.define("app-timer",r);
...
"use strict";var t=require("lit-element"),e=require("../utils.js");class r extends t.LitElement{static get styles(){return t.css`
    ...
    `}get timer(){return this._timer}set timer(t){this._timer=t,this.update()}constructor(){super(),this._timer=null,window.addEventListener("tick",t=>{0==this._timer.state&&0==this._timer.duration?this.setAttribute("state","stopped"):1==this._timer.state&&this.setAttribute("state","running"),this.update()})}render(){const r=this.timer,i=(0,e.getTimerTime)(r);return t.html`
      <div class="card-wrapper">
        ...
        <div class="state">
          ${1===r.state?t.html`<svg xmlns="http://www.w3.org/2000/svg" width="50" height="60" viewBox="0 0 50 60"><g transform="translate(-692.714 -1280.057)"><path d="M0,0,50,30h0L0,60Z" transform="translate(692.714 1280.057)"/></g></svg>`:t.html`<svg xmlns="http://www.w3.org/2000/svg" width="55" height="66" viewBox="0 0 55 66"><g transform="translate(-0.056 -0.219)"><rect width="20" height="66" transform="translate(0.056 0.219)"/><rect width="20" height="66" transform="translate(35.056 0.219)"/></g></svg>`}          
        </div>
      </div>
